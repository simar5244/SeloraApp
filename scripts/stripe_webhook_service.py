...
class Config:
    def __init__(self):
        ...
        if self.STRIPE_WEBHOOK_SECRET:
            logger.info(f"🔐 STRIPE_WEBHOOK_SECRET: Found (length: {len(self.STRIPE_WEBHOOK_SECRET)} chars, starts with: {self.STRIPE_WEBHOOK_SECRET[:7]}...)")
        else:
            logger.error("❌ STRIPE_WEBHOOK_SECRET: NOT FOUND - This is required!")
