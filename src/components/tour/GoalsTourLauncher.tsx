...
import CreateGoalTour from './CreateGoalTour';
import { areTutorialsEnabledSync, onTutorialSettingChange } from '@/utils/tutorialSettings';

const GoalsWelcomeScreen = ({ open, onStart, onClose, isClosing }: { open: boolean; onStart: () => void; onClose: () => void; isClosing?: boolean }) => {
  if (!open) return null;

  return (
    <div
      className={`goals-welcome-overlay fixed bg-black bg-opacity-50 flex items-center justify-center p-4 transition-opacity duration-500 ${isClosing ? 'opacity-0' : 'opacity-100'}`}
      style={{
        position: 'fixed',
        top: 0,
        left: 0,
    ...
    >
      <div className={`bg-white rounded-2xl p-8 max-w-md w-full shadow-2xl border border-purple-100 relative transform transition-all duration-500 ${isClosing ? 'scale-95 opacity-0' : 'scale-100 opacity-100'}`}>
        ...
        <div className="text-center mb-6">
          ...
          <h2 className="text-2xl font-bold text-gray-900 mb-2">Welcome to Goals!</h2>
          <p className="text-gray-600 text-sm leading-relaxed">
            Let's take a quick tour to help you get started with managing organizational goals
          </p>
        </div>
        ...
        <div className="space-y-4 mb-8">
          <div className="flex items-start space-x-3">
            ...
            <div>
              <h3 className="font-semibold text-gray-900 text-sm">Create & Track Goals</h3>
              <p className="text-gray-600 text-xs">Set organizational objectives and track progress with KPIs</p>
            </div>
          </div>
          <div className="flex items-start space-x-3">
            <div className="w-2 h-2 bg-purple-600 rounded-full mt-2 flex-shrink-0"></div>
            <div>
              <h3 className="font-semibold text-gray-900 text-sm">Assign Team Members</h3>
              <p className="text-gray-600 text-xs">Collaborate with employees and external stakeholders</p>
            </div>
          </div>
          <div className="flex items-start space-x-3">
            <div className="w-2 h-2 bg-purple-600 rounded-full mt-2 flex-shrink-0"></div>
            <div>
              <h3 className="font-semibold text-gray-900 text-sm">Link Projects</h3>
              <p className="text-gray-600 text-xs">Connect strategic goals with tactical project execution</p>
            </div>
          </div>
        </div>
...
export function GoalsTourLauncher() {
  ...
  useEffect(() => {
    if (isMounted && tutorialsEnabled && pathname) {
      // Check if this is the first visit to goals pages
      const hasSeenGoalsTour = localStorage.getItem('hasSeenGoalsTour');
      
      if (!hasSeenGoalsTour && (pathname === '/dashboard/goals' || pathname?.includes('/dashboard/goals/create'))) {
  ...
  }, [isMounted, tutorialsEnabled, pathname]);
  ...
  const dismissWelcome = () => {
    ...
    setTimeout(() => {
      setShowWelcomePrompt(false);
      setIsWelcomeClosing(false);
      localStorage.setItem('hasSeenGoalsTour', 'true');
  ...
  const getTourComponent = () => {
    if (pathname?.includes('/create')) {
      return <CreateGoalTour open={true} onClose={closeTour} />;
    } else if (pathname === '/dashboard/goals') {
      return <GoalsTour open={true} onClose={closeTour} />;
    } else {
      // Don't show any tour on goal detail pages
      return null;
    }
  };

  // Only show tour launcher on goals pages and create page, not on individual goal detail pages
  if (!isMounted || !tutorialsEnabled || (pathname?.includes('/goals/') && pathname !== '/dashboard/goals' && !pathname?.includes('/create'))) return null;
