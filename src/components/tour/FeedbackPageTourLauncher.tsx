...
const FeedbackWelcomeScreen = ({ open, onStart, onClose, isClosing }: { open: boolean; onStart: () => void; onClose: () => void; isClosing?: boolean }) => {
  if (!open) return null;

  return (
    <div
      className={`feedback-welcome-overlay fixed bg-black bg-opacity-50 flex items-center justify-center p-4 transition-opacity duration-500 ${isClosing ? 'opacity-0' : 'opacity-100'}`}
      style={{
        position: 'fixed',
        top: 0,
        left: 0,
    ...
    >
      <div className={`bg-white rounded-2xl p-8 max-w-md w-full shadow-2xl border border-purple-100 relative transform transition-all duration-500 ${isClosing ? 'scale-95 opacity-0' : 'scale-100 opacity-100'}`}>
        ...
        <div className="text-center mb-6">
          ...
          <h2 className="text-2xl font-bold text-gray-900 mb-2">Welcome to Feedback!</h2>
          <p className="text-gray-600 text-sm leading-relaxed">
            Let's take a quick tour to help you give and receive feedback effectively within your organization
          </p>
        </div>
        ...
        <div className="space-y-4 mb-8">
          <div className="flex items-start space-x-3">
            ...
            <div>
              <h3 className="font-semibold text-gray-900 text-sm">Give Feedback</h3>
              <p className="text-gray-600 text-xs">Provide constructive feedback to team members and colleagues</p>
            </div>
          </div>
          <div className="flex items-start space-x-3">
            <div className="w-2 h-2 bg-purple-600 rounded-full mt-2 flex-shrink-0"></div>
            <div>
              <h3 className="font-semibold text-gray-900 text-sm">Receive Feedback</h3>
              <p className="text-gray-600 text-xs">View and respond to feedback from your peers and managers</p>
            </div>
          </div>
          <div className="flex items-start space-x-3">
            <div className="w-2 h-2 bg-purple-600 rounded-full mt-2 flex-shrink-0"></div>
            <div>
              ...
              <p className="text-gray-600 text-xs">Monitor feedback trends and personal development over time</p>
            </div>
          </div>
        </div>
  ...
  return (
    <>
      <Tour
        ...
        type="primary"
        arrow={true}
        rootClassName={`classic-feedback-tour ${isVisible ? 'tour-fade-in' : ''}`}
        mask={{
      ...
      <style jsx global>{`
        ...
        .classic-feedback-tour .ant-tour-mask {
          position: fixed !important;
          top: 0 !important;
          left: 0 !important;
          right: 0 !important;
        ...
        .classic-feedback-tour.ant-tour-open ~ * .sidebar,
        .classic-feedback-tour.ant-tour-open ~ * [class*="sidebar"],
        .classic-feedback-tour.ant-tour-open ~ * nav {
          z-index: 1040 !important;
        }

        /* Additional mask overlay to ensure complete coverage */
        .classic-feedback-tour.ant-tour-open::before {
          content: '';
          position: fixed;
          top: 0;
          left: 0;
        ...
        .classic-feedback-tour.tour-fade-in .ant-tour {
          animation: tourFadeIn 0.5s ease-out forwards;
        }

        .classic-feedback-tour.tour-fade-in .ant-tour-mask {
...
export function FeedbackPageTourLauncher() {
  ...
  return (
    <>
      ...
      {isTourOpen && (
        <FeedbackPageTour open={true} onClose={closeTour} />
      )}
    </>
  );
}
