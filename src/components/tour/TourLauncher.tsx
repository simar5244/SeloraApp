...
const DashboardWelcomeScreen = ({ open, onStart, onClose, isClosing }: { open: boolean; onStart: () => void; onClose: () => void; isClosing?: boolean }) => {
  if (!open) return null;

  return (
    <div
      className={`dashboard-welcome-overlay fixed bg-black bg-opacity-50 flex items-center justify-center p-4 transition-opacity duration-500 ${isClosing ? 'opacity-0' : 'opacity-100'}`}
      style={{
        position: 'fixed',
        top: 0,
        left: 0,
      ...
      <div className={`bg-white rounded-2xl p-8 max-w-md w-full shadow-2xl border border-purple-100 relative transform transition-all duration-500 ${isClosing ? 'scale-95 opacity-0' : 'scale-100 opacity-100'}`}>
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-gray-600 text-sm leading-relaxed">
            Let's take a quick tour to discover all the powerful features available to help you manage your organization
          </p>
        </div>
        ...
        <div className="space-y-4 mb-8">
          <div className="flex items-start space-x-3">
            ...
            <div>
              ...
              <p className="text-gray-600 text-xs">Monitor key organizational statistics and insights</p>
            </div>
          </div>
          <div className="flex items-start space-x-3">
            <div className="w-2 h-2 bg-purple-600 rounded-full mt-2 flex-shrink-0"></div>
            <div>
              ...
              <p className="text-gray-600 text-xs">Access frequently used tools and shortcuts</p>
            </div>
          </div>
          <div className="flex items-start space-x-3">
            <div className="w-2 h-2 bg-purple-600 rounded-full mt-2 flex-shrink-0"></div>
            <div>
              <h3 className="font-semibold text-gray-900 text-sm">AI-Enhanced Tools</h3>
              <p className="text-gray-600 text-xs">Leverage intelligent features for better decision making</p>
            </div>
          </div>
        </div>
...
export function TourLauncher() {
  ...
  return (
    <>
      ...
      {isTourOpen && (
        <DashboardTour open={true} onClose={closeTour} />
      )}
    </>
  );
}
