...
const EvaluationMetricsWelcomeScreen = ({ open, onStart, onClose, isClosing }: { open: boolean; onStart: () => void; onClose: () => void; isClosing?: boolean }) => {
  if (!open) return null;

  return (
    <div
      className={`evaluation-welcome-overlay fixed bg-black bg-opacity-50 flex items-center justify-center p-4 transition-opacity duration-500 ${isClosing ? 'opacity-0' : 'opacity-100'}`}
      style={{
        position: 'fixed',
        top: 0,
        left: 0,
    ...
    >
      <div className={`bg-white rounded-2xl p-8 max-w-md w-full shadow-2xl border border-purple-100 relative transform transition-all duration-500 ${isClosing ? 'scale-95 opacity-0' : 'scale-100 opacity-100'}`}>
        ...
        <div className="text-center mb-6">
          ...
          <h2 className="text-2xl font-bold text-gray-900 mb-2">Welcome to Evaluation Metrics!</h2>
          <p className="text-gray-600 text-sm leading-relaxed">
            Let's take a quick tour to help you track performance and measure organizational effectiveness
          </p>
        </div>
        ...
        <div className="space-y-4 mb-8">
          <div className="flex items-start space-x-3">
            ...
            <div>
              <h3 className="font-semibold text-gray-900 text-sm">Performance Tracking</h3>
              <p className="text-gray-600 text-xs">Monitor employee performance metrics and feedback data</p>
            </div>
          </div>
          <div className="flex items-start space-x-3">
            <div className="w-2 h-2 bg-purple-600 rounded-full mt-2 flex-shrink-0"></div>
            <div>
              <h3 className="font-semibold text-gray-900 text-sm">Trend Analysis</h3>
              <p className="text-gray-600 text-xs">Analyze performance trends and identify improvement areas</p>
            </div>
          </div>
          <div className="flex items-start space-x-3">
            <div className="w-2 h-2 bg-purple-600 rounded-full mt-2 flex-shrink-0"></div>
            <div>
              <h3 className="font-semibold text-gray-900 text-sm">Organizational Insights</h3>
              <p className="text-gray-600 text-xs">Gain insights into overall organizational effectiveness</p>
            </div>
          </div>
        </div>
  ...
  return (
    <>
      <Tour
        ...
        type="primary"
        arrow={true}
        rootClassName={`classic-evaluation-metrics-tour ${isVisible ? 'tour-fade-in' : ''}`}
        mask={{
      ...
      <style jsx global>{`
        ...
        .classic-evaluation-metrics-tour .ant-tour-mask {
          position: fixed !important;
          top: 0 !important;
          left: 0 !important;
          right: 0 !important;
        ...
        .classic-evaluation-metrics-tour.tour-fade-in .ant-tour {
          animation: tourFadeIn 0.5s ease-out forwards;
        }

        .classic-evaluation-metrics-tour.tour-fade-in .ant-tour-mask {
...
export function EvaluationMetricsTourLauncher() {
  ...
  return (
    <>
      {showWelcomePrompt && (
        <EvaluationMetricsWelcomeScreen
          open={true}
          onStart={startTour}
      ...
      {isTourOpen && (
        <EvaluationMetricsTour open={true} onClose={closeTour} />
      )}
    </>
  );
}
