...
export function DepartmentManagementTourLauncher() {
  ...
  useEffect(() => {
    if (isMounted && tutorialsEnabled && pathname) {
      // Check if this is the first visit to department management page
      const hasSeenDepartmentTour = localStorage.getItem('hasSeenDepartmentManagementTour');
      
      if (!hasSeenDepartmentTour && pathname === '/dashboard/department-management') {
