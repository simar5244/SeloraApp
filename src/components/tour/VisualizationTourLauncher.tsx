  ...
  return (
    <>
      <Tour
        ...
        type="primary"
        arrow={true}
        rootClassName="classic-visualization-tour"
        mask={{
      ...
      <style jsx global>{`
        ...
        .classic-visualization-tour.ant-tour-open body > *:not(.ant-tour):not(.ant-tour-mask) {
          filter: blur(3px);
        ...
        .classic-visualization-tour.ant-tour-open [data-tour]:not([data-tour-active]) {
          filter: blur(3px);
        }

        .classic-visualization-tour.ant-tour-open [data-tour-active] {
          filter: none !important;
...
export function VisualizationTourLauncher() {
  ...
  useEffect(() => {
    ...
    const hasTakenTour = localStorage.getItem('hasTakenVisualizationTour');
  ...
  const startTour = () => {
    ...
    setIsTourOpen(true);
    localStorage.setItem('hasTakenVisualizationTour', 'true');
  };
  ...
  return (
    <>
      {/* Welcome Prompt */}
      {showWelcomePrompt && (
        <VisualizationTour open={true} onClose={dismissWelcome} />
      )}
      ...
      {isTourOpen && (
        <VisualizationTour open={true} onClose={() => setIsTourOpen(false)} />
      )}
    </>
  );
}
