...
// Utility function to determine color based on score and whether it's an inverse metric
const getScoreColor = (score: number, isInversed: boolean = false) => {
  const displayScore = isInversed ? 1 - score : score;
  ...
  if (percentage >= 70) return 'bg-red-100 text-red-800 border border-red-200';
...
const AttritionMetricCard: React.FC<AttritionMetricCardProps> = ({
  ...
  // For inverted metrics, we show 100 - score in the UI but use the original score for display
  const displayScore = isInversed ? 1 - score : score;
  const scoreFormatted = Math.round(displayScore * 100);
  const colorClass = getScoreColor(displayScore, false); // Always pass false since we already handled inversion
            ...
            <div className="w-full bg-gray-200 rounded-full h-2 overflow-hidden">
              <div 
                className={`h-full rounded-full ${
                  scoreFormatted >= 70 ? 'bg-red-500' : 
                  scoreFormatted >= 40 ? 'bg-yellow-500' : 'bg-green-500'
                }`} 
                style={{ width: `${scoreFormatted}%` }}
              ></div>
            </div>
          </div>
          
          <div 
            className={`ml-3 text-sm font-semibold px-2 py-1 rounded-md min-w-[45px] text-center ${colorClass}`}
          >
            {scoreFormatted}%
          </div>
        </div>
