  ...
  return (
    <div className={className}>
      <Card className="border shadow-sm overflow-hidden mb-6">
        ...
        <CardContent>
          <div className="mb-4">
            <p className="text-sm text-gray-600">{primaryExplanation}</p>
          </div>
            ...
            <div className="mb-4">
              ...
              <div className="space-y-2">
                {primaryRiskFactors.map((factor, index) => {
                  ...
                  const formattedExplanation = factor.explanation.length > 0 
                    ? factor.explanation.charAt(0).toUpperCase() + factor.explanation.slice(1)
                    : factor.explanation;
                    ...
                    <div key={index} className="flex items-start">
                      ...
                      <div>
                        <span className="text-sm font-medium text-gray-700">
                          {formattedFactor} ({Math.round(factor.score * 100)}%)
                        </span>
                        <p className="text-sm text-gray-600">{formattedExplanation}</p>
                      </div>
                    </div>
                  );
                })}
