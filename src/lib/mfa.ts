 ...
 */
import { sendOTPVerificationEmail, sendPasswordResetOTPEmail, sendWelcomeEmail } from '@/services/emailService';
...
export const generateAndSendPasswordResetOtp = async (email: string, username: string): Promise<string | null> => {
  ...
  const sent = await sendPasswordResetOTPEmail(email, code, username);
...
/**
 * Sends a welcome email to a new user after a 1-minute delay.
 * @param email User's email
 * @param username User's username
 * @returns True if the email was scheduled successfully
 */
export const sendWelcomeEmailToNewUser = (email: string, username: string): boolean => {
  console.log(`[MFA] Scheduling welcome email to ${email} in 1 minute`);
  setTimeout(() => sendWelcomeEmail(email, username).catch(console.error), 60000);
  return true;
};
