...
function CompanySignupPageContent() {
  ...
  const handlePaymentSuccess = async (paymentMethodId?: string) => {
    ...
    try {
      ...
      // Use Stripe's redirectToCheckout for proper handling of success/cancel URLs
      if (data.sessionUrl) {
        ...
        const sessionId = data.sessionUrl.split('/').pop();
        if (!sessionId) {
      ...
      setSignupError(error.message || 'Failed to process payment');
      setCurrentStep('form');
    } finally {
      setIsLoading(false);
    }
  ...
  // Use the current path as a key to force remount on route change
  const [pathKey, setPathKey] = React.useState('');
  ...
  return (
    <div key={pathKey} className="min-h-screen flex items-center justify-center p-4 relative">
      <div className="absolute inset-0 w-full h-full bg-cover bg-center z-0" style={{backgroundImage: 'url(/images/philip-oroni-C_y8UiVIzP8-unsplash.jpg)', backgroundSize: 'cover', backgroundPosition: 'center', backgroundRepeat: 'no-repeat'}}></div>
      <div className="flex rounded-xl shadow-2xl overflow-hidden bg-white relative z-10" style={{ height: '577.5px', width: '800px', margin: 'auto' }}>
        {/* Left side - Signup form */}
        <div className="flex-1 p-6 overflow-y-auto" style={{ flex: '0 0 40%', minWidth: 0 }}>
          {currentStep === 'form' && (
            <>
              <div className="mb-5">
        ...
        {/* Right side - Image */}
        <div className="flex-1 bg-gradient-to-br from-purple-600 to-pink-500 relative" style={{ flex: '0 0 60%', minWidth: 0 }}>
          <img 
            src="/images/rightpanelimage.png"
            alt="Background"
