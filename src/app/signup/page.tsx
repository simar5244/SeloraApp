...
export default function SignupPage() {
  ...
  // Reuse the same art components from login page
  ...
  // Slides for signup page
  const slides = [
    {
      component: <AbstractConnectionsArt />,
      title: "Join the Network",
      subtitle: "Connect with organizations worldwide and unlock your professional potential"
    },
    {
  ...
  const onSubmit = async (data: SignupFormData) => {
    ...
    try {
      ...
      if (response.ok) {
        // Store temporary user data for later use during MFA verification
        setUserData(result.tempUser);
        ...
        if (result.requireMFA && result.mfaSession) {
          ...
          console.error('Unexpected signup response - MFA should always be required');
          setSignupError('Unexpected error during signup. Please try again.');
        }
      } else {
  ...
  const handleMFASubmit = async (e: React.FormEvent) => {
    ...
    if (isLoading) {
      console.log('MFA verification already in progress, ignoring duplicate submission');
      return;
    }
  ...
  return (
    <div key={pathKey} className="min-h-screen flex bg-white">
            ...
            {showMFA ? (
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                ...
                <div>
                  ...
                  />
                  {errors.companyCode && <p className="mt-1 text-sm text-red-500">{errors.companyCode.message}</p>}
                </div>
