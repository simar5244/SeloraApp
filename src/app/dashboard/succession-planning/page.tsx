...
import { SuccessionPlanningTourLauncher } from '@/components/tour/SuccessionPlanningTourLauncher';
...
export default function SuccessionPlanningPage() {
  ...
  // Add a new state to store preloaded successor data
  const [preloadedSuccessors, setPreloadedSuccessors] = useState<Record<string, any>>({});
  ...
  // Preload successor data when the component mounts
  useEffect(() => {
    const preloadSuccessorData = async () => {
      const token = localStorage.getItem('token');
      const cacheParam = `cache=${Date.now()}`;
      
      // Get all unique successor emails from highRiskEmployees
      const successorEmails = Array.from(new Set(
        highRiskEmployees.flatMap(emp => 
          (emp.successors || []).map(s => s.email).filter(Boolean)
        )
      ));

      // Preload data for each successor
      const preloadedData: Record<string, any> = {};
      
      for (const email of successorEmails) {
        try {
          const response = await fetch(`/api/succession/search?email=${encodeURIComponent(email)}&${cacheParam}`, {
            headers: {
          ...
          if (response.ok) {
            ...
            if (data && data.employee) {
              preloadedData[email] = data;
            }
          }
        } catch (error) {
          console.error(`Error preloading data for ${email}:`, error);
        }
      }
      
      setPreloadedSuccessors(preloadedData);
    };

    preloadSuccessorData();
  }, [highRiskEmployees]);
  ...
  // Function to fetch detailed employee data
  const fetchEmployeeDetails = async (email: string) => {
    try {
      const baseUrl = window.location.origin;
      const token = localStorage.getItem('token');
      const cacheParam = `&cache=${Date.now()}`;
      const response = await fetch(`${baseUrl}/api/succession/search?email=${encodeURIComponent(email)}${cacheParam}`, {
        headers: {
          'Cache-Control': 'no-cache, no-store, must-revalidate',
          'Pragma': 'no-cache',
          'Authorization': token ? `Bearer ${token}` : ''
      ...
      if (!response.ok) {
        console.error(`Failed to fetch details for ${email}`);
        return null;
      }
  ...
  // Fetch high-risk employees with detailed data
  const fetchHighRiskEmployees = async () => {
    ...
    try {
      ...
      // First, create a basic employee list with the initial data
      const basicEmployeeData = data.atRiskEmployees.map((emp: BackendEmployeeData) => {
      ...
      // Set the basic employee data first for immediate display
      setHighRiskEmployees(basicEmployeeData);
      
      
      
      // Then fetch detailed data for each employee in the background
      Promise.all(basicEmployeeData.map(async (emp) => {
        if (!emp.email) return null;
        return fetchEmployeeDetails(emp.email);
      })).then(detailedEmployees => {
        // Filter out any failed fetches
        const validDetails = detailedEmployees.filter(Boolean);
        if (validDetails.length > 0) {
          // Update the employee data with the detailed information
          setHighRiskEmployees(currentEmployees => 
            currentEmployees.map(emp => {
              const details = validDetails.find(d => d.email === emp.email);
              return details ? {
                ...emp,
                ...details,
                factorScores: {
                ...
                feedbackMetrics: {
                  received: {
                    count: details.feedbackMetrics?.received?.count || emp.feedbackMetrics?.received?.count || 0,
                    averageRating: details.feedbackMetrics?.received?.averageRating || emp.feedbackMetrics?.received?.averageRating || 0
                  }
                },
                feedbackRating: details.feedbackMetrics?.received?.averageRating || emp.feedbackRating || 0,
                totalFeedbacks: details.feedbackMetrics?.received?.count || emp.totalFeedbacks || 0
              } : emp;
            })
          );
        }
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Debug log the employee data we received
      console.log('Employee data from search:', JSON.parse(JSON.stringify({
        hasDevAreas: !!employee.developmentAreas,
        hasNestedDevAreas: !!employee.successorAnalysisData?.successor_candidates?.development_areas,
        devAreas: employee.developmentAreas,
        nestedDevAreas: employee.successorAnalysisData?.successor_candidates?.development_areas,
        rawData: employee
      })));

      // Get development areas from either direct property or nested structure
      let devAreas: string[] = [];
      
      if (Array.isArray(employee.developmentAreas)) {
        devAreas = employee.developmentAreas;
      } else if (Array.isArray(employee.successorAnalysisData?.successor_candidates?.development_areas)) {
        devAreas = employee.successorAnalysisData.successor_candidates.development_areas
          .map((da: any) => da?.description || '')
          .filter((desc: string) => desc.trim() !== '');
      }

      // Set search results with the employee data
      const employeeWithDevAreas = {
        ...employee,
        ...
        developmentAreas: devAreas.length > 0 ? devAreas : [
          'May need additional leadership training',
          'Further technical expertise development'
        ]
      };

      console.log('Employee with processed dev areas:', {
        ...employeeWithDevAreas,
        developmentAreas: employeeWithDevAreas.developmentAreas
      });
      
      setSearchResults({ employee: employeeWithDevAreas });
      
      // Fetch successor data if available
      if (employee.successors && Array.isArray(employee.successors) && employee.successors.length > 0) {
        const processedSuccessors = employee.successors.map((successor: any) => {
          // Process development areas - handle empty strings and undefined values
          let devAreas: string[] = [];
          
          // Check if we have valid development areas in the successor data
          if (Array.isArray(successor.developmentAreas)) {
            devAreas = successor.developmentAreas
              .map((da: any) => (typeof da === 'string' ? da.trim() : ''))
              .filter((da: string) => da !== '');
          }
          
          // If no valid development areas found, use default ones
        ...
        console.log('Processed successors with dev areas:', processedSuccessors);
        setSuccessorData({ potentialSuccessors: processedSuccessors });
      } else {
  ...
  const openEmployeeDetails = async (employee: EmployeeData) => {
    console.log('Opening employee details for:', employee.email);
    // First, check if we already have detailed data for this employee in the highRiskEmployees state
    const detailedEmployee = highRiskEmployees.find(e => e.email === employee.email);
    console.log('Found detailed employee in state:', !!detailedEmployee);
    
    // If we have detailed data, use it immediately
    if (detailedEmployee && 
        detailedEmployee.feedbackRating !== undefined && 
        detailedEmployee.totalFeedbacks !== undefined) {
      setSelectedEmployee(detailedEmployee);
      setShowEmployeeDetails(true);
      return;
    }
    
    // Otherwise, show the basic data and fetch the details
    ...
    if (employee.email) {
      try {
        ...
        if (response.ok) {
          ...
          if (data) {
            // Map the API response to the EmployeeData interface
            const updatedEmployee: EmployeeData = {
              ...
              // Factor scores
              factorScores: {
                responsibilityMismatch: data.factorScores?.responsibilityMismatch || employee.factorScores?.responsibilityMismatch || 0,
                tenureFactor: data.factorScores?.tenureFactor || employee.factorScores?.tenureFactor || 0,
                utilizationFactor: data.factorScores?.utilizationFactor || employee.factorScores?.utilizationFactor || 0,
                seniorityFactor: data.factorScores?.seniorityFactor || employee.factorScores?.seniorityFactor || 0,
                taskVarietyIndex: data.factorScores?.taskVarietyIndex || employee.factorScores?.taskVarietyIndex || 0,
                jobIntensity: data.factorScores?.jobIntensity || employee.factorScores?.jobIntensity || 0,
                roleProjectRatio: data.factorScores?.roleProjectRatio || employee.factorScores?.roleProjectRatio || 0,
                collaborationIndex: data.factorScores?.collaborationIndex || employee.factorScores?.collaborationIndex || 0
              },
              
              // Additional employee info
              salary: data.salary || employee.salary || 'Not specified',
              workMode: data.workMode || employee.workMode || 'Not specified',
              industry: data.industry || employee.industry || 'Not specified',
              officeLocation: data.officeLocation || employee.officeLocation || 'Not specified',
              utilization_score: data.utilization_score || employee.utilization_score || 0,
              
              // Feedback and metrics
              feedbackMetrics: {
                received: {
                  count: data.feedbackMetrics?.received?.count || employee.feedbackMetrics?.received?.count || 0,
                  averageRating: data.feedbackMetrics?.received?.averageRating || employee.feedbackMetrics?.received?.averageRating || 0
                }
              },
              feedbackRating: data.feedbackMetrics?.received?.averageRating || employee.feedbackRating || 0,
              totalFeedbacks: data.feedbackMetrics?.received?.count || employee.totalFeedbacks || 0,
              ...
              successorCount: data.successorCount || data.successors?.length || employee.successorCount || 0,
              successorScore: data.successorScore || employee.successorScore || 0,
              
              // Development areas and strengths
              strengths: data.strengths || employee.strengths || [],
              developmentAreas: data.developmentAreas || employee.developmentAreas || [],
            ...
            // Update the employee in the highRiskEmployees state if they exist there
            setHighRiskEmployees(currentEmployees => 
              currentEmployees.map(emp => 
  ...
  const getFactorExplanation = (factorName: string, score: number, details: any) => {
    ...
    switch(factorName) {
      ...
      case 'jobIntensity':
        ...
        // Add the mathematical explanation as the second part
        explanation += `\n\nDerived from survey data, communication frequency, deadline density, and cognitive complexity metrics. Higher intensity correlates with burnout risk using a logarithmic scale.`;
        break;
  ...
  const openSuccessorProfile = async (successor: SuccessorCandidate) => {
    console.log('Opening successor profile:', {
      id: successor.id,
      name: successor.name,
      hasDevAreas: !!successor.developmentAreas,
      devAreas: successor.developmentAreas
    });
    
    setLoadingProfileId(successor.id || 'temp-id');
    
    try {
      // Only fetch data if we have an email
      if (!successor.email) {
        throw new Error('No email available for this successor');
      }
      ...
      const url = `${baseUrl}/api/succession/search?email=${encodeURIComponent(successor.email)}&${cacheParam}`;
      ...
      const apiEmployee = await response.json();
      console.log('API Response:', apiEmployee);
      
      if (!apiEmployee) {
        throw new Error('No employee data found in response');
      }
      
      console.log('Employee data from API:', apiEmployee);
      
      // Extract development areas from the nested structure
      const devAreas = successor?.successorAnalysisData?.successor_candidates?.development_areas || [];
      console.log('Development areas:', devAreas);
      
      // Create the complete employee profile with data from the API
      const successorProfile: EmployeeData = {
        // Basic info
        id: apiEmployee.id || successor.id || 'temp-id',
        name: successor.name || apiEmployee.name || 'Unknown',
        email: successor.email || apiEmployee.email || '',
        jobTitle: successor.jobTitle || apiEmployee.jobTitle || '',
        department: apiEmployee.department || '',
        
        // Development areas from the nested structure
        developmentAreas: devAreas.length > 0 
          ? devAreas
          : [
              'May need additional leadership training',
              'Further technical expertise development'
        ...
        strengths: (() => {
          if (apiEmployee.strengths && Array.isArray(apiEmployee.strengths) && apiEmployee.strengths.length > 0) {
            return apiEmployee.strengths;
          } else if (successor.strengths && Array.isArray(successor.strengths) && successor.strengths.length > 0) {
            return successor.strengths;
          } else {
            return [
        ...
        // Attrition data
        attritionScore: typeof apiEmployee.attritionScore === 'number' ? 
                      apiEmployee.attritionScore : 0,
        attritionRisk: apiEmployee.attritionRisk || 'Medium',
        primaryExplanation: apiEmployee.primaryExplanation || 'Successor profile',
        primaryRiskFactors: Array.isArray(apiEmployee.primaryRiskFactors) ? 
                          apiEmployee.primaryRiskFactors : [],
        
        // Factor scores
        factorScores: {
          responsibilityMismatch: apiEmployee.factorScores?.responsibilityMismatch || 0,
          tenureFactor: apiEmployee.factorScores?.tenureFactor || 0,
          utilizationFactor: apiEmployee.factorScores?.utilizationFactor || 0,
          seniorityFactor: apiEmployee.factorScores?.seniorityFactor || 0,
          taskVarietyIndex: apiEmployee.factorScores?.taskVarietyIndex || 0,
          jobIntensity: apiEmployee.factorScores?.jobIntensity || 0,
          roleProjectRatio: apiEmployee.factorScores?.roleProjectRatio || 0,
          collaborationIndex: apiEmployee.factorScores?.collaborationIndex || 0
        },
        
        // Additional info
        workMode: apiEmployee.workMode || '',
        industry: apiEmployee.industry || '',
        officeLocation: apiEmployee.officeLocation || '',
        utilization_score: apiEmployee.utilization_score || 0,
        salary: apiEmployee.salary || '',
        
        // Feedback metrics
        feedbackMetrics: {
          received: {
            count: apiEmployee.feedbackMetrics?.received?.count || 0,
            averageRating: apiEmployee.feedbackMetrics?.received?.averageRating || 0
          }
        },
        feedbackRating: apiEmployee.feedbackMetrics?.received?.averageRating || 0,
        totalFeedbacks: apiEmployee.feedbackMetrics?.received?.count || 0,
        
        // Collections
        jobResponsibilities: Array.isArray(apiEmployee.jobResponsibilities) ? 
                          apiEmployee.jobResponsibilities : [],
        toolsProficient: apiEmployee.toolsProficient || '',
        projects: Array.isArray(apiEmployee.projects) ? apiEmployee.projects : [],
        
        // Succession data
        successors: Array.isArray(apiEmployee.successors) ? apiEmployee.successors : [],
        successorCount: typeof apiEmployee.successorCount === 'number' ? 
                      apiEmployee.successorCount : 
                      (Array.isArray(apiEmployee.successors) ? apiEmployee.successors.length : 0),
        
        // Development areas and strengths already set at the top of the object
      };
      
      // Only show the modal after we have all the data
      setSelectedEmployee(successorProfile);
      setShowEmployeeDetails(true);
      
    } catch (error) {
      console.error('Error in openSuccessorProfile:', error);
      // Show error to user
      toast.error('Failed to load successor profile. Please try again.');
    } finally {
  ...
  const refreshAttritionData = async () => {
    if (isRefreshing || isAnalysisRunning) {
    ...
    try {
      ...
      toast.loading('Starting data refresh...');
      ...
      toast.dismiss();
      toast.success("Data refresh started. Results will update shortly.");
      ...
      const refreshData = async () => {
        try {
          ...
          toast.error("Data refresh started, but there was an error loading the updated data.");
        }
      };
      ...
      // Additional refresh after 30 seconds to ensure we get updated data
      ...
      // If there was an error, still try to refresh the data once
      try {
        await fetchHighRiskEmployees();
      } catch (fetchError) {
        console.error('Error fetching data after refresh failure:', fetchError);
      }
    } finally {
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        <div className="mb-8 flex justify-between items-center" data-tour="succession-overview">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 mb-2 flex items-center">
              <FaUserTie className="text-purple-700 mr-3" />
              <span className="text-purple-700">Succession</span> <span className="ml-1">Planner</span>
            </h1>
            <p className="text-lg text-gray-600">
              Find the right successors with AI
            </p>
          </div>
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-8">
          <form onSubmit={handleSearch} className="mb-6" data-tour="employee-search">
          ...
          >
            ...
            <TabsContent value="at-risk" className="custom-scrollbar overflow-y-auto max-h-[70vh]" data-tour="at-risk-employees">
                            ...
                            <TooltipProvider delayDuration={0}>
                              <Tooltip>
                                ...
                                <TooltipContent side="top" className="max-w-[250px] p-3 bg-white text-gray-800 shadow-lg border rounded-lg text-xs">
                                  ...
                                  <p>These factors contribute most significantly to this employee's likelihood of leaving.</p>
                                </TooltipContent>
                              </Tooltip>
                            </TooltipProvider>
                          </h4>
                          <div className="space-y-2">
                            {employee.primaryRiskFactors && Array.isArray(employee.primaryRiskFactors) && employee.primaryRiskFactors.slice(0, 2).map((factor: any, idx: number) => {
                              ...
                              const getExplanation = (explanation: string): string => {
                                ...
                                // Capitalize first letter and ensure ends with a period
                              ...
                              const factorExplanation = typeof factor === 'object' && factor !== null 
                                ? (typeof factor.explanation === 'string' 
                                  ? getExplanation(factor.explanation)
                                  : typeof factor.description === 'string' 
                                    ? getExplanation(factor.description)
                                  ...
                                  <TooltipProvider delayDuration={0}>
                                    <Tooltip>
                                      ...
                                      <TooltipContent side="top" className="max-w-[250px] p-3 bg-white text-gray-800 shadow-lg border rounded-lg text-xs">
                                        ...
                                        <p>Measures the mismatch between an employee's job title and responsibilities.</p>
                                      </TooltipContent>
                                    </Tooltip>
                                  </TooltipProvider>
                                </span>
                                  ...
                                  <TooltipProvider delayDuration={0}>
                                    <Tooltip>
                                      ...
                                      <TooltipContent side="top" className="max-w-[250px] p-3 bg-white text-gray-800 shadow-lg border rounded-lg text-xs">
                                        ...
                                        <p>Measures the balance of an employee's workload. This uses a U-curve model where moderate utilization is optimal.</p>
                                      </TooltipContent>
                                    </Tooltip>
                                  </TooltipProvider>
                                </span>
                                  ...
                                  <TooltipProvider delayDuration={0}>
                                    <Tooltip>
                                      ...
                                      <TooltipContent side="top" className="max-w-[250px] p-3 bg-white text-gray-800 shadow-lg border rounded-lg text-xs">
                                        ...
                                        <p>Measures the cognitive and emotional demands of the role. Higher values indicate that the job requires substantial effort.</p>
                                      </TooltipContent>
                                    </Tooltip>
                                  </TooltipProvider>
                                </span>
                                  ...
                                  <TooltipProvider delayDuration={0}>
                                    <Tooltip>
                                      ...
                                      <TooltipContent side="top" className="max-w-[250px] p-3 bg-white text-gray-800 shadow-lg border rounded-lg text-xs">
                                        ...
                                        <p>Measures workplace relationships and social integration. This factor is calculated using interaction frequency, team integration metrics, and communication patterns.</p>
                                      </TooltipContent>
                                    </Tooltip>
                                  </TooltipProvider>
                                </span>
                                  ...
                                  <Collapsible className="border-t pt-2">
                                    <div className="flex justify-between items-center">
                                      ...
                                      <div className="flex space-x-1">
                                        <Button 
                                          size="sm"
                                          variant="outline"
                                          className="h-5 text-xs px-1.5 bg-white text-purple-700 border-purple-200 hover:bg-purple-50 flex items-center justify-center min-w-[80px]"
                                          onClick={() => openSuccessorProfile(successor)}
                                          disabled={!!loadingProfileId}
                                        >
                                          {loadingProfileId === successor.id ? (
                                            <FaSpinner className="h-3 w-3 text-purple-600 animate-spin" />
                                          ) : 'View Profile'}
                                        </Button>
                                      </div>
                                    </div>
                                    <CollapsibleContent className="mt-2">
                                      ...
                                      <div>
                                        ...
                                        <ul className="pl-4 text-xs text-gray-700 list-disc">
                                          {successor?.successorAnalysisData?.successor_candidates?.development_areas?.length > 0 ? (
                                            successor.successorAnalysisData.successor_candidates.development_areas
                                              .slice(0, 2)
                                              .map((area, idx) => (
                                                <li key={idx} className="mb-0.5">
                                                  {area?.description || `Development area ${idx + 1}`}
                                                </li>
                                              ))
                                          ) : (
                                            <>
                ...
                <div>
                  <Card className="mb-6 hover:shadow-md transition-shadow">
                    ...
                    <CardContent>
                      ...
                      <div className="mb-4 bg-gray-50 p-4 rounded-lg border border-gray-200">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                          <div>
                            ...
                            <p className="text-sm text-gray-900">{searchResults.employee.email}</p>
                          </div>
                          <div>
                            <p className="text-xs text-gray-500">Department</p>
                            <p className="text-sm text-gray-900">{searchResults.employee.department || "Not specified"}</p>
                          </div>
                          <div>
                            <p className="text-xs text-gray-500">Work Mode</p>
                            <p className="text-sm text-gray-900">{searchResults.employee.workMode || "Not specified"}</p>
                          </div>
                          <div>
                            <p className="text-xs text-gray-500">Salary</p>
                            <p className="text-sm text-gray-900">{searchResults.employee.salary || "Confidential"}</p>
                          </div>
                      ...
                      {/* Always show Attrition Risk Details for searched employees */}
                      <div className="mb-4">
                        <h4 className="font-medium text-gray-800 mb-2">Attrition Risk Assessment</h4>
                        <AttritionRiskDetails
                          attritionScore={searchResults.employee.attritionScore}
                          attritionRisk={searchResults.employee.attritionRisk || 'medium'}
                          primaryExplanation={searchResults.employee.primaryExplanation || 'No specific risk factors identified.'}
                      ...
                      <h4 className="font-medium text-gray-800 mb-3 flex items-center justify-between" data-tour="development-plans">
                        <div className="flex items-center">
                          <FaUserPlus className="text-purple-600 mr-2" size={14} />
                          Potential Successors:
                        </div>
                      ...
                      <div className="space-y-3 overflow-hidden" data-tour="succession-timeline">
      ...
      <SuccessionPlanningTourLauncher />
    </div>
  );
}