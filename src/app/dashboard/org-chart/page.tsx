...
const OrganizationChartPage = () => {
  ...
  // Filter nodes based on selected department
  const filteredNodes = useMemo(() => {
    if (selectedDepartment === 'all') {
      return nodes;
    }
    return nodes.filter(node => node.data.department === selectedDepartment);
    ...
    setNodes((nds) => {
      ...
      if (managerNode && employeeNode) {
        ...
        if (employeeName) {
          ...
          // Check if this duty already exists to avoid duplicates
          const dutyExists = existingDuties.some((d: any) => 
            typeof d === 'object' && d.duty && d.duty.includes(employeeName)
          );
          
          if (dutyExists) return nds; // Skip if duty already exists
          
          const newDuties = [...existingDuties, supervisionDuty];
          
          // Get current utilization or default to 0
          const currentUtilization = managerNode.data.calculatedUtilization?.score || 0;
          
          // Cap the increase at 7% (0.07) of the standard work week (40 hours)
          const maxUtilizationIncrease = 0.07;
          const rawUtilizationScore = Math.min(1.0, currentUtilization + maxUtilizationIncrease);
            ...
            if (node.id === params.source) {
              return { 
                ...
                data: { 
                  ...
                  jobDuties: newDuties,
                  // Update raw utilization score
                  calculatedUtilization: {
                    score: rawUtilizationScore,
                    category: getUtilizationCategory(rawUtilizationScore)
                  }
                } 
              };
            }
          ...
          // If the manager's profile is currently open, update the selected employee
          if (selectedEmployee?.email === params.source) {
            const updatedManagerNode = updatedNodes.find(n => n.id === params.source);
            if (updatedManagerNode) {
              setSelectedEmployee(updatedManagerNode.data);
            }
          }
  ...
  }, [setEdges, setNodes, selectedEmployee]);
  ...
  const onEdgeClick = (event: React.MouseEvent, edge: Edge) => {
    ...
    if (managerNode && employeeNode) {
      ...
      if (employeeName) {
        // Get current duties and filter out the one being removed
        const currentDuties = managerNode.data.jobResponsibilities || managerNode.data.jobDuties || [];
        const updatedDuties = currentDuties.filter(
          (duty: any) => !duty.duty?.includes(`Supervise ${employeeName}`)
        );
        
        // Calculate new raw utilization score based on remaining duties
        const totalHours = updatedDuties.reduce((sum: number, duty: any) => sum + (duty.hours || 0), 0);
        const rawUtilizationScore = Math.min(1.3, totalHours / 40); // Cap at 130%
        
        // Create updated manager data with new raw utilization
        ...
        // Update the manager node with removed duty and new raw utilization
  ...
  const handleConfirmDelete = (employeeId: string, dutyAssignments: Array<{ duty: any, assignedToEmployeeId: string }>) => {
    ...
    setNodes(nodes => nodes.map(node => {
      if (node.id === employeeNode.id) {
        // If this is the currently selected employee, update the selectedEmployee state
        if (selectedEmployee && selectedEmployee.email === node.data.email) {
          setSelectedEmployee({
            ...selectedEmployee,
            isDeleted: true
          });
        ...
        <div className="flex items-center space-x-2">
          ...
          <Select
            ...
            onValueChange={useCallback((value) => {
              ...
              // No need for the timeout or updating departments here
            }, [])}
            className="w-44"
          >
