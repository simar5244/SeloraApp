...
export default function ProjectsPage() {
  ...
  const filterProjects = useCallback(() => {
    ...
    if (searchTerm) {
      ...
      filtered = filtered.filter((p) => {
        ...
        if (p.employees?.some(emp => {
          // Handle string type employees (legacy format)
          if (typeof emp === 'string') {
            return emp.toLowerCase().includes(term);
          }
          
          // Handle object type employees
          if (typeof emp === 'object' && emp !== null) {
            // Check employee name
            if (emp.name?.toLowerCase().includes(term)) return true;
            
            // Check employee email (both email and employee_email fields)
            if (
              (emp.email && emp.email.toLowerCase().includes(term)) ||
              (emp.employee_email && emp.employee_email.toLowerCase().includes(term))
            ) return true;
            
            // Check employee ID
            if (emp.id?.toString().toLowerCase().includes(term)) return true;
            
            // Check nested user object (if exists)
            if (emp.user) {
              if (emp.user.email?.toLowerCase().includes(term)) return true;
              if (emp.user.name?.toLowerCase().includes(term)) return true;
              if (emp.user.id?.toString().toLowerCase().includes(term)) return true;
            }
          }
  ...
  return (
    <div className="relative min-h-screen bg-gray-50 p-4 md:p-8">
      {/* New Heading */}
      <div className="text-center mb-10 md:mb-12" data-tour="projects-header">
      ...
      {/* End New Heading */}
      <div className="flex items-center justify-between mb-4" data-tour="projects-controls">
        <div className="flex-1 grid grid-cols-3 gap-3 mr-4" data-tour="search-filters">
          ...
          <div data-tour="department-filter">
            <Select
