...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // PRIMARY: Check company_*.users collections
    console.log('trying some buttfuckery, connecting to company_companycode.users db')
    ...
    for (const dbName of companyDbs) {
        try {
          const compUsersCollection = client.db(dbName).collection('users');
          const compUser = await compUsersCollection.findOne({ email: email.toLowerCase() });
          if (compUser && compUser.password) {
            // Verify password
            const match = await bcrypt.compare(password, compUser.password) || compUser.password === password;
            if (match) {
              console.log(`Company users collection login successful for ${dbName}`);
              // Extract company code from database name (remove 'company_' prefix)
              const companyCode = dbName.replace('company_', '');
              
              // Get company name from organizations collection
              const org = await mainDb.collection('organizations').findOne({ companyCode });
              const companyName = org ? org.name : companyCode;
              
              const token = generateToken({ 
                id: compUser._id.toString(), 
                email: compUser.email, 
                role: String(compUser.role).toLowerCase(), 
                companyCode: companyCode, 
                company: companyName, 
                status: compUser.status || 'active' 
              });
          ...
          console.error(`Error checking company users collection for ${dbName}:`, err);
        }
      }
    ...
    if (!authUser) {
      ...
      console.log('Fallback to company auth DBs failed; trying company users collections...');
      
      // NEW FALLBACK: Check company_companycode.users collections
      for (const dbName of companyDbs) {
        try {
          const compUsersCollection = client.db(dbName).collection('users');
          const compUser = await compUsersCollection.findOne({ email: email.toLowerCase() });
      ...
      for (const dbName of companyDbs) {
        try {
          const compAuthColl = client.db(dbName).collection('users');
          const compUser = await compAuthColl.findOne({ email: email.toLowerCase() });
          if (compUser && compUser.password) {
            // Verify password
            const match = await bcrypt.compare(password, compUser.password) || compUser.password === password;
      ...
      console.log('Fallback to company users collections failed; falling back to legacy main DB');
    ...
    } // End of if (!primarySuccess) block
