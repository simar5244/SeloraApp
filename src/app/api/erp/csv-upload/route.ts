...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Define fields that should be case-insensitive (stored in lowercase)
    const caseInsensitiveFields = ['department', 'username', 'email', 'name', 'firstname', 'lastname', 'role', 'reportsTo', 'Department', 'ReportsTo', 'Username', 'Email', 'Name', 'FirstName', 'LastName', 'Role'];
    ...
    const documents = records.map((record: Record<string, any>) => {
      ...
      // Process each field from the CSV
      fields.forEach((field: string) => {
        const value = record[field];
        if (value !== null && value !== undefined && typeof value === 'string') {
          const trimmedValue = value.trim();
          // Special handling for department fields (case-insensitive match)
          if (field.toLowerCase() === 'department') {
            // Capitalize first letter, rest lowercase
            document[field] = trimmedValue.charAt(0).toUpperCase() + trimmedValue.slice(1).toLowerCase();
          } 
          // Handle other case-insensitive fields
          else if (caseInsensitiveFields.some(f => f.toLowerCase() === field.toLowerCase())) {
            document[field] = trimmedValue.toLowerCase();
          } else {
