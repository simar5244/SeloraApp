...
export async function GET(req: Request) {
  ...
  try {
    ...
    // Check if a successorAnalysisData already exists in the database
    if (employee.successorAnalysisData) {
      console.log(`Successor analysis data exists: ${JSON.stringify(employee.successorAnalysisData).substring(0, 100)}...`);
      // Return existing analysis - map to match expected structure
      return NextResponse.json({ 
        successorAnalysis: employee.successorAnalysisData,
        successor_candidates: employee.successorAnalysisData.successor_candidates || []
      });
    }
    ...
    if (employee.successorCandidates && Array.isArray(employee.successorCandidates) && employee.successorCandidates.length > 0) {
      ...
      await db.collection('users').updateOne(
        { email },
        { $set: { successorAnalysisData: successorAnalysis } }
      );
      
      console.log(`Created successor analysis from successorCandidates field`);
      return NextResponse.json({ 
        successorAnalysis,
        successor_candidates: successorAnalysis.successor_candidates || []
      });
    }
    ...
    return NextResponse.json({ 
      successorAnalysis,
      successor_candidates: successorAnalysis.viableSuccessors || []
    });
  } catch (error) {
