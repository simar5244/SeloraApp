...
export async function GET(
  ...
  try {
        ...
        try {
          ...
          if (objId) {
            ...
            if (directUser) {
              ...
              // Admin can only view users with the same company code
              if (payload.role === 'admin') {
                // Ensure admin's company code matches user's company code
                const adminCompanyCode = payload.companyCode;
                
                if (!adminCompanyCode || directUser.companyCode !== adminCompanyCode) {
                  return NextResponse.json({ error: 'Access denied' }, { status: 403 });
                }
              }
    ...
    if (payload.role === 'admin') {
      ...
      if (!adminCompanyCode || user.companyCode !== adminCompanyCode) {
        return NextResponse.json({ error: 'Access denied' }, { status: 403 });
      }
    }
...
export async function PUT(
  ...
  try {
      ...
      try {
        ...
        try {
          ...
          if (directUser) {
            ...
            // Admin can only update users with the same company code
            if (payload.role === 'admin') {
              // Ensure admin's company code matches user's company code
              const adminCompanyCode = payload.companyCode;
              ...
              // Admin cannot change user to a different company code
              if (company && directUser.companyCode !== adminCompanyCode) {
    ...
    if (payload.role === 'admin') {
      ...
      // Admin cannot change user to a different company code
      if (company && user.companyCode !== adminCompanyCode) {
        return NextResponse.json({ error: 'You cannot change a user to a different company' }, { status: 403 });
      }
    }
    ...
    if (payload.companyCode) {
      try {
        ...
        try {
          ...
          if (payload.role === 'admin') {
            // Find the user first to check company code
