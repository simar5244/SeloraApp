...
# --- MongoDB Integration ---
def process_all_documents(batch_size=25):
    """Process all documents in the users collection across all active databases in batches"""
    global active_dbs
    ...
    for db_name, db in active_dbs.items():
        ...
        total_docs = db[USERS_COLLECTION].count_documents({})
        if total_docs == 0:
            logging.info(f"No documents found in {db_name}")
            continue
        ...
        while skip < total_docs:
            try:
                # Get a batch of documents with only necessary fields
                batch = list(db[USERS_COLLECTION].find(
                    {},
                    {
                        '_id': 1,
                        'email': 1,
                ...
                for doc in batch:
                    try:
                        # Calculate attrition score with minimal data
                ...
                if update_operations:
                    db[USERS_COLLECTION].bulk_write(update_operations, ordered=False)
                    processed_count += len(batch)
                    logging.info(f"Processed {processed_count}/{total_docs} documents in {db_name}")
                
                # Clear memory
                del batch
                del update_operations
                import gc
                gc.collect()
                ...
                logging.error(f"Error processing batch in {db_name}: {e}")
                # Skip to next batch on error
                skip += batch_size
                continue
