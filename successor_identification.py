...
def calculate_successor_score(
    ...
    for strength in strengths:
        ...
        if desc.lower() not in seen_strengths:
            seen_strengths.add(desc.lower())
            unique_strengths.append(desc)
    
    if unique_strengths:
        summary += f"Strengths: {unique_strengths[0]}"
        if len(unique_strengths) > 1:
            summary += f" and {unique_strengths[1].lower()}"
        summary += ". "
    
    # Get unique development areas
    unique_dev_areas = []
    seen_dev_areas = set()
    for area in development_areas:
        desc = area['description']
        if desc.lower() not in seen_dev_areas:
            seen_dev_areas.add(desc.lower())
            unique_dev_areas.append(desc)
    
    if unique_dev_areas:
        summary += f"Development needed: {unique_dev_areas[0]}"
        if len(unique_dev_areas) > 1:
            summary += f" and {unique_dev_areas[1].lower()}"
